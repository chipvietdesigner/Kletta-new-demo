...
const ReportRow: React.FC<ReportRowProps> = ({ label, amount, hasInput, isTotal }) => (
  <div className={`flex justify-between items-center py-3.5 group hover:bg-[#F9FAFB] transition-colors px-4 -mx-4 rounded-lg ${isTotal ? 'border-t border-[#E5E7EB] mt-2 pt-4' : 'border-b border-[#F9FAFB] last:border-0'}`}>
    ...
    <div className="flex items-center gap-4">
      {hasInput && (
        <button className="opacity-0 group-hover:opacity-100 bg-white hover:bg-[#F9FAFB] text-[#616A6B] hover:text-[#1E6F73] px-2.5 py-1 rounded-lg text-[11px] font-medium border border-[#E5E7EB] transition-all flex items-center gap-1.5 shadow-sm transform translate-x-2 group-hover:translate-x-0">
      ...
      <div className="max-w-[880px] mx-auto px-6 py-10 pb-20">
        <div className="mb-12 text-center flex flex-col items-center">
           ...
           <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-[#E5E7EB] shadow-sm">
             <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-[#FB923C]"></div><span className="text-[12px] font-medium text-[#000000] uppercase tracking-wide">Not sent</span></div>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm p-8 md:p-12 space-y-12">
          <div>
            <div className="flex items-center justify-between mb-6 pb-2 border-b border-[#F9FAFB]"><h2 className="text-[16px] font-medium text-[#000000]">Business Income</h2><Info size={16} className="text-[#D1D5DB]" /></div>
            <div className="space-y-1 pl-1"><ReportRow label="Revenue" amount="£0.00" /><ReportRow label="Grants and subsidies received" amount="£0.00" /></div>
          </div>
        ...
        <div className="flex flex-col items-center mt-10 gap-4">
           <div className="flex gap-4">
              <button className="bg-white border border-[#E5E7EB] hover:bg-[#F9FAFB] hover:border-[#D1D5DB] text-[#000000] font-medium text-[13px] px-6 py-3 rounded-xl shadow-sm transition-all flex items-center gap-2 h-[48px]"><DownloadSimple size={16} weight="bold" />Download PDF</button>
              <button className="bg-[#F7D84A] hover:bg-[#FCD34D] text-[#0F3A3E] font-medium text-[13px] px-8 py-3 rounded-xl shadow-sm transition-all h-[48px]">Submit Assessment</button>
           </div>
        </div>
      </div>
    </div>
